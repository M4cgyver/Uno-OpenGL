...
public class Gui {
		...
		Runnable rMain = () ->								//Create a runnable class to get input
			{
				...
				renderer 	= new Render(shBasic, 800, 600);					//Make a new Renderer instance
				...
				{
					try {
						Thread.sleep(1000/144);					//Sleep and run @ 144
	...
	public static boolean Update()
	{
		...
		renderer.render(eTable, shBasic);
		renderer.render(eFloor, shBasic);
		
		ArrayList<ModleTexture> cardsToDraw = new ArrayList<ModleTexture>();
		if(Lobby.phCurrent == Lobby.PHASE.GAME)															//Check if we are in game
		{
			Player p = Lobby.pdata.players.get(Lobby.pdata.currentPlayer);								//Get the current card
			for(int i = 0; i < p.hand.size(); i++)														//Loop through all the cards in the hand
			{
				Card currentCard = p.hand.get(i);  														//The current card
				if(currentCard.getClass() == NumberCard.class)											//The Number card
				{
					cardsToDraw.add(eCards.get(currentCard.value));
				}else if(currentCard.getClass() == ReverseCard.class)									//The reverse card
				{
					cardsToDraw.add(eCards.get(10));
				}
				else if(currentCard.getClass() == AttackCard.class)										//The attack card
				{
					cardsToDraw.add(eCards.get(11));
				}else if(currentCard.getClass() == WildCard.class)										//The wild card
				{
					cardsToDraw.add(eCards.get(12));
				}else if(currentCard.getClass() == DrawFourCard.class)									//The +4 card
				{
					cardsToDraw.add(eCards.get(13));
				}else if(currentCard.getClass() == SkipCard.class)										//The skip card
				{
					cardsToDraw.add(eCards.get(14));
				}
				else																					//There wasnt a card found?
				{
					cardsToDraw.add(eCards.get(0));
				}
		...
		{
			cardsToDraw = new ArrayList<ModleTexture>();
			for(int i = 0; i < 7; i++) cardsToDraw.add(eCards.get(0));
			fElapsed += .25f;
		...
		{
			float xoffset = ((float)i/(float)handSize)*4;
			xoffset -= 2;
			
			ModleTexture tCard = cardsToDraw.get(i);
			
			Entity e = new Entity(tCard, new Vector3f(xoffset,-4.25f,-6f-((float)i/100f)),-45/2/2,90,0,1);
			renderer.render(e, shBasic);
		}
